...
def ge_text(message):
    ...
    photo_name=text_on_img(text=result, size=300, 
    str_lehgth=max_length, str_width=width, id=message.from_user.id)
...
def text_on_img(text, size, str_lehgth, str_width, id):
    ...
    photo_name = str(id) +"_"+ datetime.datetime.now().strftime("%d_%m%_Y_%H_%M") + '.png'
    img.save(photo_name)
